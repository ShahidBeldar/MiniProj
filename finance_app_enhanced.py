    ...
    if st.session_state["analysis_history"]:
        for i, hist in enumerate(st.session_state["analysis_history"][-5:]):
            with st.expander(f"{hist['ticker']} - {hist['timestamp'][:10]}"):
                ...
                st.write(f"**Classification:** {hist['category']}")
                st.write(f"**Confidence:** {hist['confidence']:.2%}")
...
if st.session_state["show_guide"]:
    ...
    <div class="section-card">
        <h2 class="section-header">How to Use This Application</h2>
        <div class="guide-box">
            ...
            <div class="guide-step">
                ...
                <div class="step-content">
                    <h4>Run Analysis</h4>
                    <p>Click "Run Analysis" to process the headline. The system will analyze sentiment, check relevance, fetch stock data, and generate comprehensive insights.</p>
                </div>
            </div>
            <div class="guide-step">
                ...
                <div class="step-content">
                    ...
                    <p>Review the classification (Strong Buy/Buy/Hold/Sell/Strong Sell), confidence scores, technical indicators, and AI-powered trading recommendations.</p>
                </div>
            </div>
        </div>
    </div>
...
if st.session_state["current_analysis"] is not None:
    ...
    with kpi4:
        st.metric("Relevance Score", f"{result['relevance_score']*100:.1f}%")
            ...
            if stock_df is not None and len(stock_df) >= 20:
                ...
                with rec_col1:
                    ...
                    <div style="text-align: center; padding: 2rem;">
                        <div style="font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 0.5rem;">Recommendation</div>
                        <div style="font-size: 2.5rem; font-weight: 800; color: {rec_color}; margin: 1rem 0;">{recommendation}</div>
                        <div style="font-size: 0.9rem; color: #64748b;">Confidence Score: {abs(recommendation_score)}/6</div>
                    </div>
                    """, unsafe_allow_html=True)
