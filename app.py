...
for col, (title, body, pg, lbl, accent) in zip(grid, _cards):
    with col:
        st.markdown(f"""
        ...
        """, unsafe_allow_html=True)
        if st.button(lbl, key=f"_home_card_{pg}", use_container_width=True, type="secondary"):
            st.switch_page(pg)
...
for col, pg, lbl in _quick:
    with col:
        if st.button(lbl, key=f"_home_quick_{pg}", use_container_width=True):
            st.switch_page(pg)
...
for col, pg, lbl in _quick:
    with col:
        st.page_link(pg, label=lbl, use_container_width=True)
