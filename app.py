...
def generate_executive_summary(result, ticker, stock_df):
    ...
    if category == 'STRONG_POSITIVE':
        impact_text = "strongly positive"
        action = "consider aggressive long positions"
    elif category == 'POSITIVE':
        impact_text = "moderately positive"
        action = "consider long positions"
    elif category == 'STRONG_NEGATIVE':
        impact_text = "strongly negative"
        action = "consider defensive positions or short opportunities"
    elif category == 'NEGATIVE':
        impact_text = "moderately negative"
        action = "exercise caution and consider reducing exposure"
    else:
        impact_text = "neutral"
        action = "maintain current positions"
    
    relevance_text = "highly relevant" if relevance > 0.7 else "moderately relevant" if relevance > 0.4 else "minimally relevant"
    
    summary = f"""Analysis of {ticker} (Current: {price_str}) indicates a {impact_text} market impact 
    with {confidence*100:.1f}% confidence. The news is {relevance_text} to this security. 
    Based on sentiment analysis and technical indicators, investors should {action}. 
    This assessment combines AI-powered sentiment detection with real-time market data and historical patterns."""
...
if st.session_state["current_analysis"] is not None:
    ...
    time_diff = datetime.now() - analysis_timestamp
    minutes_ago = int(time_diff.total_seconds() / 60)
    freshness_text = f"Live Data" if minutes_ago < 1 else f"Updated {minutes_ago}m ago"
    
    st.markdown(f"""
        <div class="executive-summary">
            <div class="executive-summary-header">
                Executive Summary
                <span class="data-freshness">{freshness_text}</span>
            </div>
            <div class="executive-summary-content">
                {generate_executive_summary(result, ticker, stock_df)}
            </div>
        </div>
