...
if st.session_state["current_analysis"] is not None:
    ...
    tab_impact, tab_technical, tab_sentiment, tab_comparison, tab_raw = st.tabs([
        "Market Impact", 
        "Technical Analysis",
        "Sentiment History",
        "Comparison",
        ...
        with export_col2:
            if stock_df is not None:
                ...
                try:
                    ...
                    # Convert datetime columns to string to avoid Excel errors
                    stock_df_export = stock_df.copy()
                    for col in stock_df_export.columns:
                        if pd.api.types.is_datetime64_any_dtype(stock_df_export[col]):
                            stock_df_export[col] = stock_df_export[col].astype(str)
                    
                    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                        stock_df_export.to_excel(writer, sheet_name='Price Data', index=False)
