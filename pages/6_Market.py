...
FIX: Removed duplicate st.set_page_config() — was crashing on every visit.
FIX: render_sidebar() called for consistent navigation.
FIX: Index prices fetched in parallel via stocks.get_index_prices().
Tabs:
  1. Overview    — global indices ticker band + sparklines
  2. Chart       — candlestick / line + MA / BB / VWAP / Volume overlays
...
# NOTE: NO st.set_page_config() here — app.py owns the single global call.
...
def _indices():
    return get_index_prices()   # parallel fetch inside stocks.py
...
with st.sidebar:
    ...
    period     = st.selectbox("Period",     list(PERIOD_OPTIONS.keys()), index=2, key="_mkt_period")
    chart_type = st.radio("Chart Type",     ["Candlestick", "Line"], horizontal=True, key="_mkt_ct")
...
with t1:
    ...
    # Quick snapshot
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="fi-section">Quick Snapshot — Tracked Tickers</div>',
                unsafe_allow_html=True)
    ...
    for i, t in enumerate(snap_tickers):
        ...
        with snap_cols[i]:
            ...
            <div style="background:#0F1520;border:1px solid #1A2535;border-radius:10px;
              ...
              <div style="font-size:.7rem;color:{col};">{fmt_change(pd_)}</div>
            </div>
            """, unsafe_allow_html=True)
...
with t2:
    ...
    if df.empty:
        st.warning(f"No data available for **{ticker}** — try a different period.")
        st.stop()
    ...
    if show_vwap and "VWAP" in df.columns:
        fig.add_trace(go.Scatter(
            ...
            line=dict(color="#FF7D35", width=1.2, dash="dashdot"),
    ...
    if show_vol and "Volume" in df.columns:
        ...
        vol_colors= ["#00E8A0" if closes[i] >= opens[i] else "#FF3D60" for i in range(len(closes))]
        fig.add_trace(go.Bar(
            x=df.index, y=df["Volume"], name="Volume",
            marker_color=vol_colors, marker_opacity=0.5,
            hovertemplate="Vol: %{y:,.0f}<extra></extra>",
        ), row=2 if show_vol else 1, col=1)

    fig.update_layout(
        **_ct(height=560 if show_vol else 440),
        title=dict(text=f"{ticker} · {chart_type} · {period}",
                   font=dict(size=12, color="#DDE6F0", family="Syne")),
        xaxis_rangeslider_visible=False,
        showlegend=True,
    )
    fig.update_yaxes(title_text="Price",  row=1, col=1, title_font=dict(size=9, color="#3D5268"))
    if show_vol:
        fig.update_yaxes(title_text="Volume", row=2, col=1, title_font=dict(size=9, color="#3D5268"))
    st.plotly_chart(fig, use_container_width=True,
                    config={"displayModeBar": True, "modeBarButtonsToRemove": ["autoScale2d"]})
...
with t3:
    ...
    if dfi.empty:
        ...
        with ic1: st.markdown(stat_box("RSI (14)",       f"{rsi_val:.1f}",   rsi_lbl,              rsi_clr),              unsafe_allow_html=True)
        with ic2: st.markdown(stat_box("MACD",           f"{macd_v:+.4f}",   f"Signal {sig_v:.4f}","#00E8A0" if macd_v >= 0 else "#FF3D60"), unsafe_allow_html=True)
        with ic3: st.markdown(stat_box("MACD Histogram", f"{hist_v:+.4f}",   "Momentum",           "#00E8A0" if hist_v >= 0 else "#FF3D60"), unsafe_allow_html=True)
        with ic4: st.markdown(stat_box("ATR (14)",       f"{atr_v:.4f}",     f"Volatility {atr_pct:.2f}%", "#9B6DFF"), unsafe_allow_html=True)
        ...
        if "MACD" in dfi.columns:
            ...
            fig_macd.add_trace(go.Scatter(x=dfi.index, y=dfi["MACD"], mode="lines",
                name="MACD",   line=dict(color="#00C8F0", width=1.6),
                hovertemplate="MACD: %{y:.4f}<extra></extra>"), row=1, col=1)
            fig_macd.add_trace(go.Scatter(x=dfi.index, y=dfi["MACD_Signal"], mode="lines",
                name="Signal", line=dict(color="#FF7D35", width=1.3, dash="dash"),
                hovertemplate="Signal: %{y:.4f}<extra></extra>"), row=1, col=1)
            hist_vals   = dfi["MACD_Hist"].tolist()
            hist_colors = ["#00E8A0" if v >= 0 else "#FF3D60" for v in hist_vals]
            fig_macd.add_trace(go.Bar(x=dfi.index, y=dfi["MACD_Hist"],
                name="Histogram", marker_color=hist_colors, marker_opacity=0.7,
                hovertemplate="Hist: %{y:.4f}<extra></extra>"), row=2, col=1)
        ...
        if "ATR" in dfi.columns:
            ...
            fig_atr.add_trace(go.Scatter(
                x=dfi.index, y=dfi["ATR"], mode="lines", name="ATR (14)",
                line=dict(color="#9B6DFF", width=1.6),
                fill="tozeroy", fillcolor="rgba(155,109,255,.06)",
                hovertemplate="ATR: %{y:.4f}<extra></extra>",
            ))
            fig_atr.update_layout(
                **_ct(height=200),
                title=dict(text="ATR (14) — Average True Range",
...
with t4:
    ...
    with c_right:
        cmp_period  = st.selectbox("Period", list(PERIOD_OPTIONS.keys()), index=2, key="_cmp_period")
    ...
    if not cmp_tickers:
        ...
        with c_left:
            ...
            for i, (t, df_t) in enumerate(cmp_data.items()):
                ...
                fig_cmp.add_trace(go.Scatter(
                    x=df_t.index.tolist(), y=y, mode="lines", name=t,
        ...
        if len(close_df.columns) >= 2:
            ...
            fig_corr = go.Figure(go.Heatmap(
                z=z, x=lbls, y=lbls,
                colorscale=[[0.0, "#FF3D60"], [0.5, "#0F1520"], [1.0, "#00E8A0"]],
...
with t5:
    ...
    if info.get("error"):
        ...
        if hi52 > lo52 > 0 and cur52 > 0:
            rng_pct = max(0, min(100, (cur52 - lo52) / (hi52 - lo52) * 100))
            st.markdown(f"""
            <div class="fi-card" style="margin-top:.5rem;">
              ...
              <div style="display:flex;justify-content:space-between;font-size:.72rem;
                           color:#3D5268;margin-bottom:6px;">
              ...
              <div style="background:#1A2535;border-radius:4px;height:6px;position:relative;">
                ...
                <div style="position:absolute;left:{rng_pct:.1f}%;transform:translateX(-50%);
                             top:-4px;width:4px;height:14px;background:#fff;border-radius:2px;
