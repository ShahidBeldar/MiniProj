"""
pages/settings.py — Settings and Profile.
Email alerts feature removed entirely.
"""
...
from utils.db     import get_user_settings, update_user_settings, get_user_stats
from utils.corporate_graph import get_available_tickers

st.set_page_config(page_title="Settings · Finance Impact", layout="wide")
...
st.markdown("""
    <div style="margin-bottom:1.5rem;">
        <h1>Settings <span style="color:#00C8F0;">&amp; Profile</span></h1>
...
with tab_profile:
    ...
    with col_av:
        ...
        st.markdown(f"""
            <div style="width:90px;height:90px;border-radius:50%;
                        ...
                        margin:0 auto;border:3px solid #22334A;color:#fff;">
                {uname[0].upper()}
            </div>
        """, unsafe_allow_html=True)
            ...
            <div class="fi-card">
                <table style="width:100%;border-collapse:collapse;font-size:0.82rem;">
                    ...
                    <tr style="border-bottom:1px solid #1A2535;">
                        ...
                        <td style="padding:8px 0;color:#DDE6F0;">{stats.get('total',0)}</td>
                    </tr>
                    <tr>
    ...
    st.markdown('<div class="section-label">Analysis Breakdown</div>', unsafe_allow_html=True)
    ...
    for lbl, count, color in [
        ...
        st.markdown(f"""
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
                <div style="font-size:0.78rem;color:#7A92A8;width:70px;">{lbl}</div>
...
# ── PREFERENCES ───────────────────────────────────────────────────────────────
with tab_prefs:
    st.markdown('<div class="fi-card"><div class="fi-card-title">Analysis Defaults</div>',
                unsafe_allow_html=True)
    ...
    new_default  = st.selectbox("Default Ticker", available, index=default_idx,
...
# ── SECURITY ──────────────────────────────────────────────────────────────────
with tab_security:
    st.markdown('<div class="fi-card"><div class="fi-card-title">Change Password</div>',
                unsafe_allow_html=True)
...
with tab_about:
    st.markdown("""
        <div class="fi-card">
            <div class="fi-card-title">Platform Information</div>
            <table style="width:100%;border-collapse:collapse;font-size:0.82rem;">
                <tr style="border-bottom:1px solid #1A2535;">
