...
Works on Streamlit Cloud via st.session_state path handling
"""
...
def get_db_path() -> str:
    """Return DB path that works both locally and on Streamlit Cloud."""
...
def seed_default_users():
    """Seed admin/demo users if DB is fresh. Called on app start."""
    import bcrypt
    defaults = [
    ...
    conn.execute("""
        INSERT INTO analysis_history
            (user_id, ticker, headline, polarity, category, confidence,
             relevance_score, event_type, is_rumour, credibility,
             ripple_count, result_json)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        user_id,
        ticker,
...
def add_to_watchlist(user_id: int, ticker: str, company_name: str, notes: str = ""):
    try:
        conn = get_conn()
        conn.execute(
...
def update_user_settings(user_id: int, settings: dict):
    ...
    conn.execute("""
        INSERT OR REPLACE INTO user_settings
            (user_id, default_ticker, show_confidence, show_ripple, show_history, email_alerts, alert_email)
        VALUES (?,?,?,?,?,?,?)
    """, (
        user_id,
