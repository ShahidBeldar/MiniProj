...
if st.session_state["current_analysis"] is not None:
    ...
    with tab_impact:
        ...
        with col_cat2:
            ...
            st.markdown(f"""
            <div class="info-box">
                <p style="margin: 0; line-height: 1.6;">{result['reason']}</p>
            </div>
            """, unsafe_allow_html=True)
            
            if not result['is_relevant']:
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(239, 68, 68, 0.2) 100%); 
                     padding: 1rem; border-radius: 10px; border-left: 4px solid #ef4444; margin-top: 1rem;">
                    <p style="margin: 0; color: {'#f87171' if st.session_state['theme'] == 'dark' else '#dc2626'}; font-weight: 500;">
                        This news headline does not appear to be directly relevant to {ticker}. Market impact is expected to be minimal.
                    </p>
                </div>
                """, unsafe_allow_html=True)
