...
st.navigation() with position="sidebar" gives a persistent, always-visible sidebar.
"""
import sys, os
...
# Auth gate — show login before any page registration
if not is_logged_in():
    render_login_page()
    st.stop()

# ── SIDEBAR HEADER (logo + user chip + sign-out) ─────────────────────────────
_un    = uname()
...
# ── PAGE REGISTRY (this is what makes switch_page work on Cloud) ─────────────
pg = st.navigation(
    {
        "": [
    ...
    position="sidebar",   # Streamlit renders the real sidebar — always visible
)

# Sign-out button at bottom of sidebar
with st.sidebar:
    st.markdown(
        '<div style="height:1px;background:#1A2535;margin:.6rem .4rem .4rem;"></div>',
        unsafe_allow_html=True,
    )
